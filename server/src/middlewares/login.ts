...
function signJWT(user: UserProps, res: Response) {
...
export async function loginMiddleware(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!User) {
      ...
      const matchingPassword = await bcrypt.compare(req.body.senha, User.getDataValue("senha"));
      if (!matchingPassword) {
...
export function jwtMiddleware(req: Request, res: Response, next: NextFunction) {
  try {
    const token = cookieExtractor(req);
    if (token) {
      const decoded = jwt.verify(token, SECRET) as { user: UserProps };
